...
class MyDatasetLoader:
    ...
    def load_dataset_for_multi_label_detection(self):
        dirs_test = ['images_p', 'labels_p']
        ...
        for dir in dirs_test:
            images.append(self.unpickle('../data/raw/myDatasetDetectionMultiLabel/' + str(CURR_CHANNELS) + 'ch/' + dir))

        meta.append(self.unpickle('../data/raw/myDatasetClfsMultiLabel/' + str(CURR_CHANNELS) + 'ch/batch_meta_p'))
    ...
    def load_dataset_for_multi_label_classification(self, curr_class):
        ...
        for dir in dirs:
            data.append(self.unpickle('../data/raw/myDatasetClfsMultiLabel/' + str(CURR_CHANNELS) + 'ch/' + curr_class + dir))
    ...
    def pickle_classification_data(self):
        ...
        self.pickle(images, 'all_images_p', base_path + str(CURR_CHANNELS) + 'ch/')
        self.pickle(classes, 'labels_p', base_path + str(CURR_CHANNELS) + 'ch/')
        self.pickle(dirs, 'batch_meta_p', base_path + str(CURR_CHANNELS) + 'ch/')
    ...
    def pickle_classification_data_for_mlc(self, curr_class):
        ...
        for dir in dirs:
            ...
            if CURR_CHANNELS == 3:
                ...
                single_class = [1] * len(dataset_appended_with_dm) if dir == curr_class else [0] * len(dataset_appended_with_dm)
                classes.append(single_class)
        ...
        self.pickle(images, curr_class + '_p', base_path + 'MultiLabel/' + str(CURR_CHANNELS) + 'ch/')
        self.pickle(classes, curr_class + '_labels_p', base_path + 'MultiLabel/' + str(CURR_CHANNELS) + 'ch/')
        self.pickle([0, 1], 'batch_meta_p', base_path + 'MultiLabel/' + str(CURR_CHANNELS) + 'ch/')
    ...
    def pickle_detection_data(self):
        ...
        if CURR_CHANNELS == 3:
            ...
            self.pickle(images, 'images_p', path + str(CURR_CHANNELS) + 'ch/')
            ...
            self.pickle(images_appended_with_dm, 'images_p', path + str(CURR_CHANNELS) + 'ch/')
        ...
        with open(path + "labels", newline='') as csvfile:
            ...
            self.pickle(data, 'labels_p', path + str(CURR_CHANNELS) + 'ch/')

    def pickle_detection_data_for_mlc(self):
        path = '../data/raw/myDatasetDetection'
        ...
        if CURR_CHANNELS == 3:
            images = [cv2.imread(file) for file in filenames if file[-5:] == 'L.jpg']
            self.pickle(images, 'images_p', path + 'MultiLabel/' + str(CURR_CHANNELS) + 'ch/')

        elif CURR_CHANNELS == 4:
            images = [cv2.imread(file) for file in filenames]
            reshaped = MyDatasetHelper.resize_images(images, shape=(640, 480))
            images_appended_with_dm = MyDatasetHelper.crete_disparity_maps_serial(reshaped)
            self.pickle(images_appended_with_dm, 'images_p', path + 'MultiLabel/' + str(CURR_CHANNELS) + 'ch/')

        else:
            print('invalid channles')
            return None

        with open(path + "/labels_mlc", newline='') as csvfile:
            reader = csv.reader(csvfile, delimiter=',')
            data = []
            for row in reader:
                data.append(row)

            self.pickle(data, 'labels_p', path + 'MultiLabel/' + str(CURR_CHANNELS) + 'ch/')
